...
export function useGetPsychicReading(userName: string, enabled: boolean = false, refreshKey: number = 0) {
  ...
  return useQuery<string>({
    ...
    queryFn: async () => {
      ...
      const randomIndex = Math.floor(Math.random() * psychicReadingVariations.length);
      const reading = psychicReadingVariations[randomIndex];
      
      // Personalize the reading
      const personalizedReading = userName && userName !== 'Seeker' 
...
export function useGetHoroscope(sign: string, refreshKey: number = 0) {
  const { actor, isFetching } = useActor();
...
export function useGetLoveHoroscope(sign: string, refreshKey: number = 0) {
  const { actor, isFetching } = useActor();
