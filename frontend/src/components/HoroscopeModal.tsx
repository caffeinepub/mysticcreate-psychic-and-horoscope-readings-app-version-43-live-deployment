...
import { Loader2, Moon, Heart, RefreshCw } from 'lucide-react';
...
export default function HoroscopeModal({ isOpen, onClose, isBlueTheme = false }: HoroscopeModalProps) {
  const [selectedSign, setSelectedSign] = useState<string>('');
  const [userName, setUserName] = useState('');
  const [activeTab, setActiveTab] = useState<'daily' | 'love'>('daily');
  const [refreshKey, setRefreshKey] = useState(0);
  const [displayedDailyReading, setDisplayedDailyReading] = useState<string>('');
  const [displayedLoveReading, setDisplayedLoveReading] = useState<string>('');
  
  const { data: horoscope, isLoading: isLoadingDaily, isFetching: isFetchingDaily } = useGetHoroscope(selectedSign, refreshKey);
  const { data: loveHoroscope, isLoading: isLoadingLove, isFetching: isFetchingLove } = useGetLoveHoroscope(selectedSign, refreshKey);
  ...
  const selectedZodiac = zodiacSigns.find((sign) => sign.value === selectedSign);
  ...
  const currentReading = activeTab === 'daily' ? displayedDailyReading : displayedLoveReading;
  const showLoading = (isLoading || isFetching) && !currentReading;

  const personalizeReading = (text: string) => {
    if (!userName.trim()) return text;
    return text.replace(/You /g, `${userName}, you `)
    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className={`max-w-2xl border-2 backdrop-blur-xl transition-all duration-500 ease-in-out ${
        ...
        <div className="py-8">
          ...
          {/* Name Input */}
          <div className="mb-6">
            <Label htmlFor="horoscopeName" className={`block text-lg font-semibold mb-3 text-center transition-colors duration-500 ease-in-out ${
          ...
          <div className="mb-8">
            <Label className={`block text-lg font-semibold mb-3 text-center transition-colors duration-500 ease-in-out ${
              isBlueTheme ? 'text-blue-200' : 'text-cream'
            }`}>
              Select Your Zodiac Sign
            </Label>
            <Select value={selectedSign} onValueChange={setSelectedSign}>
              <SelectTrigger className={`w-full text-lg py-6 rounded-xl transition-all duration-500 ease-in-out ${
                isBlueTheme
              ...
              <SelectContent className={`border-2 transition-all duration-500 ease-in-out ${
                isBlueTheme
          ...
          {selectedSign && (
            <Tabs value={activeTab} onValueChange={(v) => setActiveTab(v as 'daily' | 'love')} className="w-full">
              <TabsList className={`grid w-full grid-cols-2 border transition-all duration-500 ease-in-out ${
                isBlueTheme
                  ...
                  <>
                    <div className={`rounded-2xl p-8 border backdrop-blur-sm transition-all duration-500 ease-in-out ${
                      ...
                      <div className="text-center mb-4">
                        ...
                        <h3 className={`text-2xl font-bold mt-2 capitalize transition-colors duration-500 ease-in-out ${
                          isBlueTheme ? 'text-blue-300' : 'text-gold'
                        }`}>
                          {selectedSign}
                        </h3>
