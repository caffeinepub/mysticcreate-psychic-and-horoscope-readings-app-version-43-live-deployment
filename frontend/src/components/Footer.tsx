...
export default function Footer({ isBlueTheme = false }: FooterProps) {
  ...
  return (
    <>
      <footer className={`py-8 px-4 backdrop-blur-sm border-t transition-all duration-500 ease-in-out ${
        isBlueTheme 
        ...
        <div className="container mx-auto text-center space-y-4">
          <div className="flex flex-col md:flex-row items-center justify-center gap-6 md:gap-8">
            ...
            <div className="relative order-1 md:order-1">
              ...
              >
                ...
                />
                {/* Hover overlay hint */}
                <div className={`absolute inset-0 rounded-lg transition-all duration-500 ease-in-out flex items-center justify-center ${
                  isBlueTheme ? 'bg-blue-400/0 group-hover:bg-blue-400/10' : 'bg-gold/0 group-hover:bg-gold/10'
                }`}>
                  <span className={`font-semibold text-sm opacity-0 group-hover:opacity-100 transition-opacity duration-500 ease-in-out bg-background/80 px-3 py-1 rounded-full ${
                    isBlueTheme ? 'text-blue-300 border border-blue-400/50' : 'text-bright-gold border border-gold/50'
                  }`}>
                    About Her
                  </span>
                </div>
              ...
              <div className="relative">
                <img
                  ...
                  className={`w-32 h-32 sm:w-40 sm:h-40 rounded-lg object-contain shadow-lg transition-all duration-500 ease-in-out ${
                    isBlueTheme ? 'border-2 border-blue-400/50' : 'border-2 border-gold/50'
                  }`}
                />
              </div>
              <p className={`text-lg sm:text-xl font-semibold transition-all duration-500 ease-in-out ${
                isBlueTheme ? 'text-blue-300 shimmer-text-blue' : 'text-gold shimmer-text'
              }`}>
          ...
          <p className={`flex items-center justify-center gap-2 flex-wrap text-sm sm:text-base transition-colors duration-500 ease-in-out ${
            ...
            <a
              ...
              className={`transition-colors duration-500 ease-in-out underline ${
                isBlueTheme ? 'text-blue-300 hover:text-blue-300/80' : 'text-gold hover:text-gold/80'
              }`}
            >
              caffeine.ai
            </a>
